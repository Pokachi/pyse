...
function replace_ingredient(recipe, old_name, new_name)
...
function data_util.fix_icon_size(prototypes, icon_name)
  for _, proto in pairs(prototypes) do
    if proto.icon == icon_name then
      proto.icon_size = 32
      log("Fixed icon_size for: " .. proto.name)
    end

    -- If using icons = { ... }
    if proto.icons then
      for _, icon in pairs(proto.icons) do
        if icon.icon == icon_name then
          icon.icon_size = 32
          log("Fixed layered icon_size for: " .. proto.name)
        end
      end
    end
  end
